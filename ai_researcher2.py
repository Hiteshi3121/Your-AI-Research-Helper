...
def call_model(state: State):
    ...
    try:
        ...
        # ✅ CRITICAL FIX: graceful failure instead of crash
        error_msg = (
            "⚠️ Unable to reach the language model service.\n\n"
            "This is a temporary network / DNS issue.\n"
...
IMPORTANT:
When generating a research paper:
- DO NOT include \\documentclass, \\usepackage, \\begin{document}, or \\end{document}
- Write ONLY the paper content whatever necessary (for example: title, abstract, sections, equations, conclusion, references, etc.)
- Make sure to cite references in the format [1], [2], etc., and provide a reference list at the end of the paper.
- Use Markdown-style headings (#, ##) for sections
- Use LaTeX math ONLY inside $...$
If the user asks for a PDF you MUST CALL the tool `render_latex_pdf`, and generate the full paper content.
Do not ask follow-up questions.
Do not explain.
Always produce the PDF.
"""
